...
```{r}
reading <- read_excel( , sheet = "1. reading skills")
```
...
```{r}
PA <-   # cantidad de parámetros del modelo Aumentado
PC <-   # cantidad de parámetros del modelo Compacto
...
Calcule la suma de cuadrados denominada **SSR** e identifíquele en el output de `anova()`.
