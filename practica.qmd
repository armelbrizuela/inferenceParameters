...
```{r}
reading$sex <- as.factor()
reading$group <- as.factor()
```

Elimine el símbolo "-" de los valores contenidos en la columna `time`. Para

```{r}
reading$time <- str_remove( , )
```

Es necesario reorganizar los datos para todas las observaciones de una misma persona participante aparezcan en una misma fila.

```{r}
reading <- pivot_wider(reading, names_from = time, values_from = w_speed:text_accuracy)
```
...
Muestre una tabla de frecuencias de las variables `factor`. También muestre cuántos valores `NA` hay en cada columna. Para ello, utilice la función `describirNoNumericas()`.
